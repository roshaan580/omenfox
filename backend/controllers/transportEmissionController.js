...
exports.createTransportEmission = async (req, res) => {
  try {
    ...
    const transportEmission = new TransportEmission({
      userId,
      month,
      year,
      transportMode,
...
exports.getAll = async (req, res) => {
  try {
    ...
    const transportEmission = await TransportEmission.find({ userId }).sort({
      year: -1,
      month: -1,
    });

    if (!transportEmission)
...
exports.getTransportEmission = async (req, res) => {
  try {
    ...
    const transportEmission = await TransportEmission.findOne({
      userId,
      year,
      month,
    });
